...
pub fn main() !void {
	comptime var c = Aurora.initComptime();
...
pub fn main() !void {
    var c = Aurora.initRuntime(.{ .allocator = std.heap.your_allocator });
    defer c.deinit();
	try c.green().bold().printOut("Hello, world!", .{});
}
```
...
- ### `createPreset`
  ...
  Create a new instance of Aurora by saving the current styles as a preset, to be used for defining custom themes.
